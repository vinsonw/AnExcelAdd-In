Attribute VB_Name = "阶段性报告制作Shadow"
...
Public Sub OnKeyToDoc(control As IRibbonControl)
'注意这个总过程是由汇总月报文件的过程改过来的，本过程含有汇总月报功能，其他子过程被包含在此过程尾部
...
currentUseWB.Worksheets("SourceData").Delete
...
Set srcSheet = currentUseWB.Worksheets.Add(after:=currentUseWB.Worksheets("PivotSheet"))
...
'MsgBox "写入word文档所需数据处理完毕，点击确定开始生成Word文档，注意制定的阶段性报告模板必须在此文件夹下！"
